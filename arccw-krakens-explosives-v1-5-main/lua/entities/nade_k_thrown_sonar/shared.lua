...
if SERVER then
	...
	function ENT:Detonate()
		...
		self:EmitSound( "sonar_grenade_sound_v2.wav", 75, 100, 1, CHAN_AUTO )
...
if CLIENT then
	...
	function ARC9_HaloManager:DrawHalo()
		...
		-- Draw bright red full model highlight with wireframe material
		cam.Start3D(EyePos(), EyeAngles())
			render.SetColorModulation(1, 0, 0) -- Bright red
			render.SetBlend(0.7) -- Semi-transparent
			render.ModelMaterialOverride(Material("models/wireframe"))
			for _, ent in ipairs(_ents) do
				if IsValid(ent) then
					ent:DrawModel()
				end
			end
